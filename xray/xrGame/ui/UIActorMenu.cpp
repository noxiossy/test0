...
*/
	...
	case mmTrade:
		...
		break;
	case mmUpgrade:
		m_pDeadBodyActorBagListUpd->clear_select_armament();
		break;
	case mmDeadBodySearch:
...
/*#else
	if (weapon && (item_slot == INV_SLOT_2))
	{
		m_InvSlot2Highlight->Show(true);
		return;
	}
	if (weapon && (item_slot == INV_SLOT_3))
	{
		m_InvSlot3Highlight->Show(true);
		return;
	}
...
void CUIActorMenu::set_highlight_item( CUICellItem* cell_item )
{
	...
	case mmUpgrade:
		{
			highlight_armament( item, m_pDeadBodyActorBagListUpd );
			break;
		}
	case mmTrade:
		{
